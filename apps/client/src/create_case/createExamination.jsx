...
export default function CreateExamination(props) {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            const categories = await props.getCallToApi("http://localhost:5173/api/case/getExaminationTypes", headers);
            const categoryMap = {};
                ...
                <FormControl>
                    ...
                    <VStack>
                        ...
                        {Object.entries(examinationCategories).map(([id, name]) => (
                            ...
                            <>
                                <Checkbox id={id} alignSelf='start'>{name}</Checkbox>

                                {examinationSubcategories[id].map((element) => (
                                    <Checkbox id={element.id} alignSelf='center'>{element.name}</Checkbox>
                                ))}
                            </>
